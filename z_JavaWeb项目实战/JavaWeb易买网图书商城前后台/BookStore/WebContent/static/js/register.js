	...
	switch(obj.name) {
		case "userName":
			...
			if(obj.value == "") {
				...
				$.get("UserAjaxServlet",{userName:obj.value},function(result) {
		 			//判断回调函数中的值
					eval("var res="+result);
					if(res.nameExit) {
		 				//用户名已经被占用
		...
		case "email":
			...
			if(obj.value == "") {
				...
				//校验注册邮箱是否已经存在
 	 			$.get("UserAjaxServlet",{email:obj.value},function(result){
 	 				eval("var res="+result);
 	 				if(res.emailExit) {
 	 					//用户邮箱已经存在
		...
		case "tel":
			...
			if(obj.value == "") {
				...
				//校验注册的联系电话是否已经存在
 	 			$.get("UserAjaxServlet",{tel:obj.value},function(result) {
 	 				eval("var res="+result);
 	 				if(res.telExit) {
